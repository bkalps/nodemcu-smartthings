...
1. One SmartThings Device Handler for contact sensors and motion sensors
2. One SmartThings SmartApp that interfaces with the Wifi-connected device in your home
...
![Nate's Annotated Photo](screenshots/Nates_annotated_photo.png "Nate's Annotated Photo")
...
4. Open up credentials.lua in your favorite text editor and put in your WiFi SSID and password
	* When putting your wifi credentials in "credentials.lua" I think it needs to be a 2.4GHz channel wifi. The board wouldn't recognize my 5GHz channel.
...
1. have the name LoLin on the back or front
2. the small rectangular component on the board near the USB port is engraved with CH340G
3. Mac OS X Sierra users: use this driver
...
3. The exact command Nate used to flash his firmware was:
```
...
```
4. The port may vary depending on your platform, OS and UART driver.
	* My port: cu.wchusbserial1420
	* Mode: dio for >=4 MByte modules (qio might work as well, YMMV).
	* Firmware: firmware/nodemcu-1.5.4.1-final-10-modules-2017-03-23-20-42-52-integer.bin
	* Start with sudo to have admin privileges, and use complete pathnames depending on where you are in the Terminal
5. The exact command I used to flash my firmware was:
```
...
9. During testing, the balcony door pin was spotty (initially set to D9, there was no D9, so tried D8 and now it's D4… and now D5)
	* I forgot not to use D4 because it's tied to the LED on the board. Noticed D5 wasn't being used.
...
3. Once everything is working properly, you can plug the NodeMCU into a standard USB power adapter and it will automatically boot up, connect to WiFi, and start listening for switches.
	* Or, like me, I powered it from my existing Alarm Panel since it looks cleaner:
		...
		- On my GE Concord board, I hooked up to pin 11, which is a +12V DC and pin 13, which is GND.
